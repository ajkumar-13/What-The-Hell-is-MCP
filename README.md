...
- [verify/RESULTS.md](verify/RESULTS.md) — the measured values behind
  [code/05-first-server/](code/05-first-server/), plus the two deployment edge cases post 21
  reports, regenerated by [verify/capture.py](verify/capture.py). Every other project's
  figures come from running that project's own test suite. If a post and its source disagree,
  the post is wrong. Ground truth before prose.
