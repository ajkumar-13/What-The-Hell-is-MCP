...
**Progress reporting and tasks are different mechanisms, and only one of them works everywhere today.** The `ctx.report_progress` call above is core protocol. When the client puts a `progressToken` in the request's `_meta`, the server may emit `notifications/progress` on that request's response stream, and `watch_cpu` does. That is a real method on `Context` in `mcp` 2.0.0b2, it needs no extension, and the tool above is registered and published like any other: [verify/RESULTS.md](../../verify/RESULTS.md) lists `watch_cpu` with one input property and an output schema, alongside the rest of the server's tools.
