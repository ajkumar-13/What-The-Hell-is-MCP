...
- **Forwarding the client's token upstream.** If your server calls a third-party interface, get your own token from that service's authorization server. Passing the client's token through is the confused deputy, and the specification forbids it outright.
